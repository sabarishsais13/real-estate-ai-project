...
def validate_image_size(image):
    ...
    if image.size > max_size:
        raise ValidationError(f'Image file too large. Size should not exceed 5MB. Current size: {image.size / (1024*1024):.1f}MB')
...
def validate_image_dimensions(image):
    ...
    if width < 200 or height < 200:
        raise ValidationError('Image dimensions too small. Minimum size is 200x200 pixels.')
    if width > 4096 or height > 4096:
        raise ValidationError('Image dimensions too large. Maximum size is 4096x4096 pixels.')
...
class Property(models.Model):
    ...
    image         = models.ImageField(
        ...
        help_text='Upload a property image (JPG, JPEG, PNG). Max size: 5MB. Min dimensions: 200x200px.'
    )
